...
explore_html_test_() ->
    {foreach, local, fun get_file_content/0, [fun research_01/1]}.

explore_css_test_() ->
    {foreach, local, fun get_css_file_content/0, [fun research_css_01/1]}.
...
research_xdigit_test() ->
    List = lists:seq(0, 127),
    lists:map(fun(Elem) ->
                 Text = io_lib:format("~.16B", [Elem]),
                 Regex = "[[:xdigit:]]",
